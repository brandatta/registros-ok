...
if "pendientes" not in st.session_state:
    st.session_state["pendientes"] = set()
...
# Aplicar sets de visibilidad
df_pendientes = df_pendientes[~df_pendientes["id"].isin(st.session_state["procesados"])]
df_procesados = df_procesados[~df_procesados["id"].isin(st.session_state["pendientes"])]
        ...
        with st.container():
            ...
            with cols[1]:
                if st.button("Sí", key=f"btn_si_{row['id']}"):
                    ...
                    st.session_state["procesados"].add(row["id"])
            with cols[2]:
        ...
        with st.container():
            ...
            with cols[2]:
                if st.button("No", key=f"btn_no_proc_{row['id']}"):
                    ...
                    st.session_state["pendientes"].add(row["id"])
            with cols[3]:
                st.markdown("<span style='font-size:1.5rem; color:green;'>✓</span>", unsafe_allow_html=True)
...
if st.session_state["hora_inicio"]:
    ...
    if subtotal_local > 0:
        estimado_total_min = (minutos / subtotal_local) * len(df)
